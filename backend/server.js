...
app.post("/properties", verifyToken, (req, res) => {
  ...
  db.query(
    ...
    [address, property_type, rent_amount, status],
    (e, r) => {
...
app.post("/tenants", verifyToken, (req, res) => {
  ...
  db.query(
    "INSERT INTO tenants (user_id,phone_number,id_proof) VALUES (?,?,?)",
    [user_id, phone_number, id_proof],
    (e, r) => {
      if (e) return res.status(500).json(e);
...
app.put("/tenants/:id", verifyToken, (req, res) => {
  ...
  db.query(
    "UPDATE tenants SET phone_number=?,id_proof=? WHERE tenant_id=?",
    [phone_number, id_proof, req.params.id],
    (e) => {
      if (e) return res.status(500).json(e);
...
app.post("/leases", verifyToken, (req, res) => {
  ...
  db.query(
    `INSERT INTO leases (property_id,tenant_id,start_date,end_date,rent_amount,deposit,status)
     VALUES (?,?,?,?,?,?,?)`,
    [property_id, tenant_id, start_date, end_date, rent_amount, deposit, status],
    (e, r) => {
      if (e) return res.status(500).json(e);
...
app.post("/payments", verifyToken, (req, res) => {
  ...
  db.query(
    ...
    [lease_id, amount, payment_date, month, status],
    (e, r) => {
      if (e) return res.status(500).json(e);
